...
with st.expander("🔌 Tổn thất các TBA công cộng"):
    ...
    if temp_upload_tba_thang:
        st.session_state.df_tba_thang = pd.read_excel(temp_upload_tba_thang, sheet_name=0, skiprows=6)
    ...
    if temp_upload_tba_luyke:
        st.session_state.df_tba_luyke = pd.read_excel(temp_upload_tba_luyke, sheet_name=0, skiprows=6)
    ...
    if temp_upload_tba_ck:
        st.session_state.df_tba_ck = pd.read_excel(temp_upload_tba_ck, sheet_name=0, skiprows=6)
