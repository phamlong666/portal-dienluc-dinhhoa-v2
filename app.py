...
# ============================
# 📈 TIỆN ÍCH: DỰ BÁO THEO ĐIỀU KIỆN CHỌN (CHỈ TRONG MODUL "📍 Dự báo điểm sự cố")
# ============================
            ...
            try:
                ...
                if st.button("🔍 Tra cứu"):
                    ...
                    if input_sum == 0:
                        ...
                        # Ước lượng dòng cơ sở gần nhất
                        closest_base = tra_cuu_df["Dòng tổng (A)"].sub(input_sum).abs().idxmin()
                        dong_co_so = tra_cuu_df.loc[closest_base, "Dòng cơ sở (A)"]
                        ket_qua = tra_cuu_df[
                            (tra_cuu_df["Đường dây"] == selected_line) &
                            (tra_cuu_df["Loại sự cố"] == selected_fault) &
                            (tra_cuu_df["Dòng cơ sở (A)"] == dong_co_so)
                        ]
